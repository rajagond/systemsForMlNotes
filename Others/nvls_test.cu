...
// Copied from https://github.com/microsoft/mscclpp/blob/main/test/nvls_test.cu
...
__global__ void init_kernel(float* uc_ptr, size_t size, int myrank, int nranks) {
  for (size_t idx = threadIdx.x + blockIdx.x * blockDim.x; idx < size; idx += blockDim.x * gridDim.x) {
    // Print some sample values (just print a few to avoid flooding the output)
    // uc_ptr[idx] = myrank + idx;
    uc_ptr[idx] = myrank + idx;
    if (idx < 5) {
      printf("Rank %d initialized: uc_ptr[%ld] = %f\n", myrank, idx, uc_ptr[idx]);
    }
  }
}
...
__global__ void check_correctness(float* uc_ptr, size_t size, int myrank, int nranks) {
  for (size_t idx = threadIdx.x + blockIdx.x * blockDim.x; idx < size; idx += blockDim.x * gridDim.x) {
    float expected = (float)((nranks * (nranks - 1)) / 2 + nranks * idx);
      // Print some sample values (just print a few to avoid flooding the output)
      if (idx < 5) {
        printf("Rank %d checking: uc_ptr[%ld] = %f (expected: %f)\n", 
                myrank, idx, uc_ptr[idx], expected);
      }
...
__global__ void check_correctness2(float* uc_ptr, size_t size, int myrank, int nranks) {
  for (size_t idx = threadIdx.x + blockIdx.x * blockDim.x; idx < size; idx += blockDim.x * gridDim.x) {
    float expected = (float)((nranks * (nranks - 1)) / 2 + nranks * idx);
      // Print some sample values (just print a few to avoid flooding the output)
      if (idx < 5) {
        printf("Rank %d checking2: uc_ptr[%ld] = %f (expected: %f)\n", 
                myrank, idx, uc_ptr[idx], expected);
      }
    // if (abs(uc_ptr[idx] - expected) > 0.01 * expected) {
    //   printf("error! idx %ld: %f != %f\n", idx, uc_ptr[idx], expected);
...
int main() {
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  init_kernel<<<nblocks, block_size>>>((float*)uc_va, size / sizeof(float), myrank, nranks);
  // Synchronize device to ensure check_correctness is finished
  cudaDeviceSynchronize();
  MPI_Barrier(MPI_COMM_WORLD);
  
  // Perform cudaMemcpy from UC to MC pointer (device to device)
  if (!myrank)
  {
    printf("Rank %d: Performing cudaMemcpy from UC to MC pointer\n", myrank);
    // Note: size is in bytes, so we need to cast to (float*) for the correct size
    cudaError_t err = cudaMemcpy((float*)mc_va, (float*)uc_va, size, cudaMemcpyDeviceToDevice);
    if (err != cudaSuccess) {
      printf("Rank %d: cudaMemcpy failed with error: %s\n", myrank, cudaGetErrorString(err));
    } else if (!myrank) {
      printf("Rank %d: cudaMemcpy from UC to MC completed successfully\n", myrank);
    }
  }
  
  // Synchronize device to ensure cudaMemcpy is finished
  cudaDeviceSynchronize();
  MPI_Barrier(MPI_COMM_WORLD);
  check_correctness2<<<nblocks, block_size>>>((float*)uc_va, size / sizeof(float), myrank, nranks);
  cudaDeviceSynchronize();
  MPI_Barrier(MPI_COMM_WORLD);
